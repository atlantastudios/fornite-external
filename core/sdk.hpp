...
Camera get_view_point()
{
	...
	fnrot.b = read<double>(rotation_pointer + ROTATION_OFFSET_B);
	fnrot.c = read<double>(rotation_pointer + ROTATION_OFFSET_C);
	...
	view_point.fov = read<float>(cache::player_controller + BASEFOV) * 90.0f; //ITS BASEFOV since we use player controller if u use local player then use 0x4AC
	return view_point;
}
...
Vector3 get_entity_bone(uintptr_t mesh, int bone_id)
{
	...
	FTransform bone = read<FTransform>(bone_array + (bone_id * BONE_STRIDE));
...
{
	auto Seconds = read<double>(cache::uworld + VISIBLE_SECONDS);
	auto LastRenderTime = read<float>(mesh + LAST_SUBMIT_TIME);
